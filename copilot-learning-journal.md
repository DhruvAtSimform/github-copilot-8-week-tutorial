...
- ✅ Request documentation and run books
...
- **The Skill Solution:** A PDF skill can include a script that uses a library (like Python's `py-pdf`) to extract text.
...
## Week 5 - AI Agents and MCP servers

This study guide explains the **Model Context Protocol (MCP)** based on the Code-basics tutorial. It breaks down how MCP acts as a unified interface for AI applications to interact with external tools and data.
...
## Week 6 - Collaboration, Documentation & PR reviews:

Here are the concise and comprehensive markdown notes for **GitHub Copilot Code Review** based on the official documentation for both the VS Code extension and the GitHub Web UI.

---

### 🚀 GitHub Copilot Code Review: VS Code Integration

The VS Code integration is designed for **immediate, local feedback** before you even push your code to a remote repository.

- **How to Trigger:**
- **Selection Review:** Highlight a block of code, right-click, and select **Copilot > Review and Comment**.
- **Source Control Review:** In the Source Control (Git) tab, you can request a review of all **Staged Changes** or **Unstaged Changes** by clicking the Copilot icon.

- **Key Features:**
- **Inline Feedback:** Comments appear directly in your editor, similar to "Problems" or "Lints."
- **One-Click Fixes:** Many suggestions come with a "Propose Fix" button that allows you to apply the AI's suggestion instantly.
- **Custom Focus:** You can provide specific instructions (e.g., "Focus on security" or "Check for memory leaks") through the chat interface.

- **Best For:** Catching logic errors, "code smells," and formatting issues during the active development phase.

### 🌐 GitHub Copilot Code Review: Web UI (Pull Requests)

The Web UI experience is integrated into the **GitHub Pull Request workflow**, acting as a first-pass reviewer for your team.

- **How to Trigger:**
- **Manual Request:** On the Pull Request page, open the **Reviewers** menu in the right sidebar and select **Copilot**.
- **Automatic Review:** Repository admins can configure **Rulesets** to automatically trigger a Copilot review whenever a PR is opened or updated.

- **Key Features:**
- **Full Diff Context:** Copilot reviews the entire diff of the PR, understanding how changes in one file affect others.
- **Standard PR Comments:** It leaves comments directly on the "Files Changed" tab. These comments can be replied to, resolved, or reacted to just like a human reviewer’s feedback.
- **Non-Blocking:** Copilot reviews are "Comment" only. They do not count as a required approval and will not block a merge by themselves.

- **Best For:** Standardizing code quality across a team, summarizing changes, and reducing the "noise" for human reviewers.

---

### 📊 Comparison: VS Code vs. Web UI

| Feature              | VS Code (IDE)                    | GitHub Web UI (PR)                  |
| -------------------- | -------------------------------- | ----------------------------------- |
| **Primary Use Case** | Pre-commit / Local development   | Collaborative PR workflow           |
| **Scope**            | Selected lines or staged changes | Full PR diff                        |
| **Feedback Style**   | Interactive editor comments      | Standard PR review comments         |
| **Automation**       | Manual trigger                   | Can be fully automated via Rulesets |
| **Actionability**    | Immediate "Apply Fix" buttons    | Suggestions must be committed       |
| **Visibility**       | Private to the developer         | Visible to the whole team           |

---

### 💡 When to Use Which?

- **Use VS Code Code Review when:** You are actively coding and want a "second pair of eyes" before committing. It is excellent for deep-diving into a specific function or complex logic block you just wrote.
- **Use GitHub Web UI Review when:** You are ready to share your work. It ensures that basic errors are caught before your teammates spend time looking at the code. It is also the best choice for Lead Developers who want to enforce consistent quality checks automatically.

---

### 💰 Pricing & Copilot Pro Subscription

GitHub Copilot Code Review features are part of the "Agentic" capabilities of Copilot.

- **Copilot Pro ($10/month):**
- **Individual Use:** Includes access to both VS Code and Web UI code review features.
- **Premium Requests:** You receive a monthly allowance of **300 premium requests**. Code reviews are considered "premium requests" because they use more advanced reasoning and context.
- **Usage:** Once you exhaust your 300 requests, the speed or availability of the agent-based reviews may be limited until the next cycle, or you may be prompted to buy more.

- **Copilot Business/Enterprise:** These tiers offer higher limits and centralized management, but for an individual **Pro** subscriber, the primary constraint is the 300-request monthly limit.

---

[Using GitHub Copilot for code reviews: from VS Code to github.com](https://www.youtube.com/watch?v=xf65vxjNWdk)

This video demonstrates the seamless transition of the code review process from a developer's local editor in VS Code to the final pull request stage on GitHub.

### Deliverables:

I gave the agent below prompt to migrate the express-api project to typescript from javascript.
Since project size is relatively small and prompt used the plan based approach, migration was smooth and accurate.

Prompt:

```
   You are a seasoned nodeJS and typescript developer with expertise in express.js

   Your main task is to migrate this Javascript based express-api code into latest typescript.

   Prepare the plan and todo list based on below list. Work on one task at a time and update the progress.

   tasks:
   1. Understand the current code setup and structure
   2. Add require typescript package. Try to use the current most stable version.
   3. Update and refactor the @#file:package.json scripts to work with tsx
   4. Add the missing @types for existing node packages using the pnpm
   5. Refactor the code to use proper types whenever required.
   6. Add new eslint rules for typescripts. Remove any conflicting rules for javascript.
   7. Update the #file:copilot-instructions.md. Add project overview and tech at start. Add new instructions to follow the best typescript practices for nodeJs server.
   8. At last check build is working and server is running without any linting and run-time errors.
```

- **PR generation**:

* We use the github mcp server running docker container with the Github PAT token with required permissions.
* Attempts to setup github mcp server using http and vs code extension failed with auth error and unknown type.
* Finally docker approach worked which our agent used to generate the new PR for current git repo.
* It successfully generated PR with below message:
  ![Copilot generated PR](images/pr-img.png)

- **PR Review**:

* Pr was assigned to github copilot to review it which took a minute or so to complete reviewing relatively large PR.
* 4 comments where added which were up to the mark. Package version mis matching which can go under the radar for human eyes.
* I gave the local agent a prompt to fetch the PR comments and check if it were already fixed or not. If not fix those.
* And it worked properly (!! issues were minor).

---

## Week 7: Security Review & OWASP Top 10 Audit
...
```
Act as a senior security engineer with expertise in OWASP Top 10 vulnerabilities.
Review the code and setup for a Node.js/Express application.

Identify security vulnerabilities including but not limited to: SQL injection, XSS,
CSRF, improper authentication, insecure dependencies, and sensitive data exposure.

Prepare the comprehensive report with potential risks, level, possible solutions which
can be used to review and implement changes in next iteration.
```

#### Audit Outcome

**18 vulnerabilities identified** across all severity levels:

- 🔴 **3 Critical** - Authentication gaps, rate limiting, CSRF protection
- 🟠 **5 High** - XSS, insecure CORS, unsafe CSP, missing input validation
- 🟡 **6 Medium** - Security headers, environment validation, logging gaps
- 🟢 **4 Low** - Error verbosity, API versioning, request tracking

#### Security Fixes Implemented

**10 vulnerabilities fixed** with new security middleware and configurations:

1. **Rate Limiting** (Critical) - IP + User-Agent tracking with route-specific limits
2. **CSRF Protection** (Critical) - Double Submit Cookie pattern implementation
3. **XSS Prevention** (High) - Fixed unsafe `innerHTML` in client code (3 locations)
4. **CORS Hardening** (High) - Strict origin validation (same-origin + dev localhost)
5. **CSP Improvements** (High) - Removed `unsafe-inline`, strict nonce-based policies
6. **Input Validation** (High) - Zod-based request validation middleware
7. **Security Headers** (Medium) - Comprehensive Helmet.js configuration
8. **Environment Validation** (Medium) - Zod schema for required env variables
9. **Error Handling** (Medium) - Verified stack trace suppression in production
10. **Data Sanitization** (Low) - Validated timezone data handling

**Files Created:**

- [src/middlewares/rateLimiter.ts](express-rest-api/src/middlewares/rateLimiter.ts)
- [src/middlewares/csrfProtection.ts](express-rest-api/src/middlewares/csrfProtection.ts)
- [src/middlewares/validateRequest.ts](express-rest-api/src/middlewares/validateRequest.ts)
- [src/config/corsConfig.ts](express-rest-api/src/config/corsConfig.ts)
- [src/config/securityHeaders.ts](express-rest-api/src/config/securityHeaders.ts)
- [src/config/env.ts](express-rest-api/src/config/env.ts)
- [src/validators/timezoneValidators.ts](express-rest-api/src/validators/timezoneValidators.ts)

**Documentation Generated:**

- [SECURITY_AUDIT_REPORT.md](express-rest-api/docs/SECURITY_AUDIT_REPORT.md) - Detailed vulnerability analysis with code examples
- [SECURITY_IMPLEMENTATION_SUMMARY.md](express-rest-api/docs/SECURITY_IMPLEMENTATION_SUMMARY.md) - Implementation tracking and integration guide
- [FINAL_STATUS_REPORT.md](express-rest-api/docs/FINAL_STATUS_REPORT.md) - Overall project status and decisions

### 🛡️ Security Best Practices for Node.js/NestJS

#### Essential Security Audit Prompts

**1. OWASP Top 10 Comprehensive Review**

```
Act as a security architect specializing in Node.js applications. Perform a comprehensive
OWASP Top 10 2021 security audit on this Express/NestJS application. For each category:

A01 (Broken Access Control) - Review authentication, authorization, and permission checks
A02 (Cryptographic Failures) - Audit data encryption, password hashing, TLS configuration
A03 (Injection) - Check SQL/NoSQL injection, command injection, XSS vulnerabilities
A04 (Insecure Design) - Evaluate threat modeling, secure design patterns
A05 (Security Misconfiguration) - Review headers, CORS, CSP, error handling
A06 (Vulnerable Components) - Analyze dependencies and supply chain risks
A07 (Auth Failures) - Assess session management, brute force protection, MFA
A08 (Software Integrity) - Check CI/CD pipeline security, code signing
A09 (Logging Failures) - Audit security event logging and monitoring
A10 (SSRF) - Validate URL inputs and external request handling

Provide severity ratings, exploit scenarios, and actionable fixes with code examples.
```

**2. Authentication & Authorization Deep Dive**

```
Act as an identity and access management (IAM) specialist. Review the authentication
and authorization implementation in this Node.js application. Analyze:

- JWT token security (algorithm, secret strength, expiration, refresh strategy)
- Password policy enforcement (bcrypt rounds, complexity requirements)
- Session management (httpOnly cookies, secure flags, SameSite attribute)
- Role-based access control (RBAC) implementation
- API key management and rotation
- OAuth2/OpenID Connect implementation (if applicable)
- Multi-factor authentication readiness
- Brute force protection (rate limiting, account lockout)
- Password reset flow security (token expiration, single-use tokens)

Identify vulnerabilities and provide secure implementation examples.
```

**3. Input Validation & Sanitization Audit**

```
Act as a web application security expert. Audit all user input handling in this
Express/NestJS application:

- Query parameters, route parameters, request body validation
- File upload security (type validation, size limits, virus scanning)
- Content-Type verification
- SQL/NoSQL injection prevention (parameterized queries, ORM usage)
- XSS prevention (output encoding, CSP headers)
- Path traversal protection
- Command injection prevention
- XML/JSON parsing vulnerabilities
- Use of validation libraries (Joi, Zod, class-validator)

Provide validation schemas and sanitization middleware examples.
```

**4. API Security Hardening**

```
Act as an API security consultant. Review this REST/GraphQL API for security best practices:

- Rate limiting strategy (per-endpoint, per-user, per-IP)
...
- API key vs OAuth vs JWT authentication appropriateness
- Pagination and data exposure limits
- GraphQL-specific issues (depth limiting, query complexity)
- Mass assignment vulnerabilities
- Idempotency key implementation

Provide production-ready security configurations.
```

**5. Dependency & Supply Chain Security**

```
Act as a DevSecOps engineer. Perform a supply chain security audit:

- Identify vulnerable dependencies (npm audit, Snyk, GitHub Dependabot)
- Check for outdated packages with known CVEs
- Analyze transitive dependencies risks
- Review package-lock.json integrity
- Evaluate use of deprecated packages
- Check for malicious packages (typosquatting)
- Assess private registry security (if used)
- Review CI/CD pipeline for dependency injection risks
- Suggest automated vulnerability scanning integration
- Provide upgrade strategies for breaking changes
...
```
Act as a cryptography and secrets management expert. Audit how this Node.js application
handles sensitive data:

- Environment variable security (.env files, process.env usage)
- API keys, database credentials, JWT secrets storage
- Use of secrets management services (AWS Secrets Manager, HashiCorp Vault, Azure Key Vault)
- Hardcoded secrets detection (git history scanning)
- Encryption at rest and in transit
- Key rotation strategies
- Logging of sensitive data (passwords, tokens in logs)
- Secret exposure in error messages or stack traces
- Client-side secret exposure risks
- Docker/Kubernetes secrets handling

Provide secure configuration examples and migration paths.
```

**7. Error Handling & Information Disclosure**

```
Act as a penetration tester. Analyze error handling for information disclosure vulnerabilities:

- Stack traces in production responses
- Verbose error messages revealing implementation details
- Database error exposure (connection strings, table structures)
- File path disclosure
- Version information leakage (X-Powered-By headers)
...
Ensure no leakage of technical details to potential attackers.
```

**8. NestJS-Specific Security Audit**

```
Act as a NestJS security specialist. Review NestJS-specific security patterns:

- Guards implementation (authentication, authorization)
- Pipes for validation (ValidationPipe with class-validator)
- Interceptors for logging and security headers
- Exception filters for error handling
- Middleware usage (CORS, Helmet, rate limiting)
- Dependency injection security (scope management)
- ConfigModule secrets handling
- TypeORM/Prisma security (parameterized queries, migrations)
- GraphQL security (complexity limits, depth limits)
- Microservices communication security (message validation)
- WebSocket security (authentication, CORS)

Provide NestJS best practice implementations.
```

**9. Production Deployment Security Checklist**

```
Act as a cloud security architect. Create a production readiness security checklist
for deploying this Node.js application:
...
Provide missing implementations with priority levels.
```

**10. Continuous Security Monitoring Prompt**

```
Act as a security operations center (SOC) analyst. Design a continuous security
monitoring strategy for this Node.js application:
...
- Weekly DAST (Dynamic Application Security Testing)
- Monthly penetration testing automation
- Git commit secret scanning
...
Integrate with SIEM/logging platforms and provide alerting configurations.
```

### 🎯 Key Learnings

#### Security-First Development with Copilot

1. **Comprehensive Audits**: Using role-based prompts ("Act as a security engineer...") provides deeper, expert-level analysis
2. **Documentation Generation**: Copilot excels at creating detailed security reports with categorized vulnerabilities
3. **Code + Config**: Security fixes span multiple layers - middleware, configuration, client-side code, and environment validation
4. **Iterative Review**: Breaking security review into specialized prompts (auth, input validation, dependencies) yields thorough results

#### OWASP Top 10 Mapping Strategy

- **A01 (Access Control)**: Authentication, RBAC, rate limiting
- **A02 (Cryptographic)**: Secrets management, TLS, password hashing
- **A03 (Injection)**: Input validation (Zod), parameterized queries, output encoding
- **A04 (Design)**: Threat modeling, security patterns review
- **A05 (Misconfiguration)**: Helmet headers, CORS, environment validation
- **A06 (Components)**: Dependency audits, `npm audit`, Snyk integration
- **A07 (Auth Failures)**: Session management, brute force protection, MFA
- **A08 (Integrity)**: CI/CD security, code signing
- **A09 (Logging)**: Security event logging, monitoring integration
- **A10 (SSRF)**: URL validation, allowlist enforcement

#### Practical Implementation Insights

- **Zod Validation**: Type-safe validation for both runtime (requests) and compile-time (env vars)
- **Layered Security**: Client-side (XSS fixes) + Server-side (CSRF, rate limiting) + Configuration (Helmet, CORS)
- **Decision Documentation**: Not all vulnerabilities need immediate fixes - document deferred items with justification
- **Integration Testing**: Security fixes require testing across multiple scenarios (valid requests, attack attempts, edge cases)
